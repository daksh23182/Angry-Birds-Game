...
public class PlayScreen2 implements Screen {
    ...
    private int bigPigHealth = 3;

    private Wood wood;
    private Glass glass;
    private Glass additionalGlass;

    private boolean gameOver = false;

    public PlayScreen2(AngryBirdd game, String saveFile) {
    ...
    public PlayScreen2(AngryBirdd game) {
        this.game = game;
        ...
        bigPig = new BigPig(new Texture("BigPig.png"), new Vector2(1600, 325));
        ...
        wood.setSize(new Vector2(400, 150));// Example position

        glass = new Glass(new Texture("Glass.png"), new Vector2(1500, 100));
        glass.setSize(new Vector2(75, 600));

        additionalGlass = new Glass(new Texture("Glass.png"), new Vector2(1400, 100)); // Example position
        additionalGlass.setSize(new Vector2(75, 600)); // Example size
    }
    ...
    public void render(float delta) {
        ...
        if (additionalGlass != null) {
            game.batch.draw(additionalGlass.getTexture(), additionalGlass.getX(), additionalGlass.getY(), additionalGlass.getWidth(), additionalGlass.getHeight());
        }
        if (mediumPig != null) {
    ...
    private void drawSlingLine(Vector2 birdPosition) {
        ...
        shapeRenderer.setColor(0.5f, 0.3f, 0.1f, 1); // Brown color for the sling
    ...
    private void handleCollisionDamage(Collidable object1, Collidable object2) {
        int damage = Math.min(object1.getHealth(), object2.getHealth());
    ...
    private void checkGameOver() {
        ...
        boolean isVictory = bigPig == null && mediumPig == null;
        ...
        if (isVictory) {
            ...
            System.out.println("Defeat! No more birds left!");
            gameOver = true;
    ...
    private void updateBirdPhysics(float delta) {
        ...
        if (wood != null && isCollision(birdPhysics.getPosition(), currentBird.getSize(), wood.getPosition(), wood.getSize())) {
            ...
            if (wood.isDestroyed()) {
                ...
                if (bigPig != null) {
                    System.out.println("BigPig is damaged due to falling Wood!");
                    bigPig.takeDamage(25);

                    if (bigPig.isDestroyed()) {
                        System.out.println("BigPig is destroyed due to falling Wood!");
        ...
        if (bigPig != null && isCollision(birdPhysics.getPosition(), currentBird.getSize(), bigPig.getPosition(), bigPig.getSize())) {
            System.out.println("Bird collided with BigPig!");
        ...
        if (glass != null && isCollision(birdPhysics.getPosition(), currentBird.getSize(), glass.getPosition(), glass.getSize())) {
            System.out.println("Bird collided with Glass!");
        ...
        if (additionalGlass != null && isCollision(birdPhysics.getPosition(), currentBird.getSize(), additionalGlass.getPosition(), additionalGlass.getSize())) {
            System.out.println("Bird collided with Additional Glass!");
            handleCollisionDamage(currentBird, additionalGlass);

            if (additionalGlass.isDestroyed()) {
                System.out.println("Additional Glass is destroyed!");
                additionalGlass.getTexture().dispose();
                additionalGlass = null;
            }
    ...
    private void applyDamageToBigPigOnWoodBreak() {
        if (bigPig != null) {
            bigPig.takeDamage(25);
            System.out.println("BigPig took 25 damage due to wood breaking! Remaining health: " + bigPig.getHealth());

            if (bigPig.isDestroyed()) {
                System.out.println("BigPig is destroyed due to wood breaking!");
                bigPig.getTexture().dispose();
                bigPig = null;
            }
        }
    ...
    private void displayHealthStatus() {
        ...
        if (additionalGlass != null) {
            System.out.println("Additional Glass: " + additionalGlass.getHealth());
        } else {
            System.out.println("Additional Glass: Destroyed");
        }
    ...
    public void saveLevel() {
        ...
        saveLevelState("Level2.save", birds, pigs, materials, currentBird, gameOver);
    ...
    public void loadLevel(String filename) {
        try (FileInputStream fileIn = new FileInputStream(filename);
            ...
            // Restore birds
            birdQueue.clear();
            for (Bird bird : levelState.birds) {
                bird.setTexture(new Texture(bird.texturePath)); // Reload bird texture
                birdQueue.add(bird);
            }
            ...
            for (Pig pig : levelState.pigs) {
                if (pig instanceof BigPig) {
                    bigPig = (BigPig) pig;
                    bigPig.setTexture(new Texture("BigPig.png")); // Reload texture
                    bigPig.setPosition(bigPig.getPosition());     // Restore position
                } else if (pig instanceof MediumPig) {
                    mediumPig = (MediumPig) pig;
                    mediumPig.setTexture(new Texture("MediumPig.png")); // Reload texture
            ...
            for (Material material : levelState.materials) {
                if (material instanceof Wood) {
                    wood = (Wood) material;
                    wood.setTexture(new Texture("Wood.png")); // Reload texture
                    wood.setPosition(wood.getPosition());     // Restore position
                } else if (material instanceof Glass) {
                    if (glass == null) {
                        glass = (Glass) material;
                        glass.setTexture(new Texture("Glass.png")); // Reload texture
                        glass.setPosition(glass.getPosition());     // Restore position
                    } else {
                        additionalGlass = (Glass) material;
                        additionalGlass.setTexture(new Texture("Glass.png")); // Reload texture
                        additionalGlass.setPosition(additionalGlass.getPosition()); // Restore position
                    }
                }
            }
            ...
            if (levelState.currentBird != null) {
                ...
                currentBird.setTexture(new Texture(currentBird.texturePath)); // Reload texture
